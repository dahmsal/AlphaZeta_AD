...
public class Board {
     ...
     */
    public Vector2D findSpaceship(Spaceship spaceship) throws ParameterException {
        Tile foundTile = this.board.stream()
                .filter(tile -> tile.getSpaceshipOnTile().equals(spaceship)).findFirst().orElse(null);
        if (foundTile != null) {
            return foundTile.getPosition();
        }
